...
import { CheckCircle, XCircle, ScanFace, Clock, Users, Wifi } from "lucide-react";
...
export default function GateAttendancePage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (!FaceMesh) throw new Error("MediaPipe not loaded");
        ...
        faceMesh.onResults((results) => {
          ...
          if (results.multiFaceLandmarks?.length > 0) {
            tryCapture(results.multiFaceLandmarks[0]);
          }
        });

        mpFaceRef.current = faceMesh;
        setStatus("scanning");
  ...
  const drawOverlay = (results) => {
    ...
    ctx.strokeStyle = status === "scanning" ? "#10b981" : "#fff";
    ctx.lineWidth   = 3;
  ...
  // ── Auto Capture ───────────────────────────────────
  const tryCapture = useCallback(async (landmarks) => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div className="flex flex-1 gap-0">
          ...
          <div className="flex-1 flex items-center justify-center p-6">
            <div className="relative w-full max-w-lg">
              ...
              <div className="relative bg-slate-900 rounded-2xl overflow-hidden aspect-[4/3] border border-slate-700 shadow-2xl">
                ...
                {status === "fail" && (
                  <FailFlash onDone={handleOverlayDone} />
                )}
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                <div className="bg-slate-900 rounded-xl p-3 border border-slate-700 text-center">
                  <p className="text-2xl font-bold text-emerald-400">{stats.unique}</p>
                  <p className="text-slate-400 text-xs mt-0.5">Unique Students</p>
                </div>
                <div className="bg-slate-900 rounded-xl p-3 border border-slate-700 text-center">
          ...
          <div className="w-80 border-l border-slate-800 flex flex-col">
            <div className="p-4 border-b border-slate-800 flex items-center gap-2">
              ...
              <p className="text-slate-300 font-semibold text-sm">Recent Attendance</p>
              {log.length > 0 && (
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {log.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-slate-500 text-sm">Abhi koi attendance nahi</p>
